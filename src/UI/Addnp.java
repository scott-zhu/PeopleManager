...
public class Addnp extends JFrame{
	IEmployeeDao dao;
	EmployeeServiceImp service;
	JFrame ad = new JFrame("籍贯添加");
	JPanel p = new JPanel();
	JLabel lblduty = new JLabel("籍贯名称:");
	JTextField txtduty = new JTextField(10);
	JButton btnduty = new JButton("添加籍贯");
	public Addnp(){
		...
		btnduty.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				if(duty.equals("")){
					JOptionPane.showMessageDialog(null, "请输入籍贯名称!","警告",JOptionPane.WARNING_MESSAGE);
					return;
				}
				...
				boolean flag = service.addNP(duty);
				if(flag){
					JOptionPane.showMessageDialog(null, "添加籍贯成功!","友情提示",JOptionPane.INFORMATION_MESSAGE);
					return;
				}else{
					JOptionPane.showMessageDialog(null, "添加籍贯失败!","发生错误",JOptionPane.ERROR_MESSAGE);
					return;
				}
			}
